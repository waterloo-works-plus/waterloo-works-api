  ...
  getJobById: async (page, selectedTerm, jobId) => {
    ...
    for (let i = 0; i < tables.length; i++) {
      ...
      for (let j = 0; j < rows.length; j++) {
        ...
        const valueHtml = valueCell.html();
        ...
        if (key === 'Job Posting Status:') {
          ...
          job.specialJobRequirements = valueHtml.trim();
        } else if (key === 'Job Summary:') {
          job.summary = valueHtml.trim();
        } else if (key === 'Job Responsibilities:') {
          job.responsibilities = valueHtml.trim();
        } else if (key === 'Required Skills:') {
          job.requiredSkills = valueHtml.trim();
        } else if (key === 'Compensation and Benefits Information:') {
          job.compensationAndBenefitsInformation = valueHtml.trim();
